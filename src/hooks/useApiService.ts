...
export function useApiService<T extends object>({
  service,
  initFetch,
  autoFetch,
  defaultPageSize = 6,
}: Props<T>) {
  const [response, setResponse] = useState<PaginationResponse<T>>({
    data: [],
    page: 0,
    size: 0,
    pages: 0,
  ...
  const update = useCallback(
    async (id: number, payload: Partial<T> | FormData) =>
      mutate(async () => {
