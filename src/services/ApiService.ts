...
export default class ApiService {
  ...
  public async findAll<T>({
    endpoint,
    config,
  }: { endpoint?: string; config?: AxiosRequestConfig } = {}): Promise<
  ...
  public async login({
    ...
    const res = await this.axiosInstance.post<SessionResponse>('/auth/login', {
      username,
      password,
    })
    return res.data
  ...
  public async signUp({
    ...
    const res = await this.axiosInstance.post<SessionResponse>(
      '/auth/signup',
      payload
    )
    return res.data
